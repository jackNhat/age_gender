...
def experiment(args):
    ...
    dataset = args.dataset # ['FairFace']
    network = args.network # ['resnet50', 'mobilenetv2']
    sensitive_group = args.sensitive_group # ['ageAndgender']
    prune_type = args.prune_type # ['FairGRAPE']
    ...
    epoches = [13,3,3]
    frames = make_frame(csv_file, face_dir)
    ...
    if init_train and prune_type in ['WS', 'Full', 'FairGRAPE','Lottery', 'Importance'] or print_acc:
        ...
        full_fair_df = save_output(best_model,[dataset, prune_type,prune_rate, frames['test'], face_dir, total_classes, network, col_used, output_cols_each_task, sensitive_group, exp_idx],csv_savedir, False)

    print("Iters to prune:", prune_iters)
    if prune_type in pruner_map:
        prunner = pruner_map[prune_type](best_model, criterion, train_loader,output_cols_each_task, save_mask)
    ...
    for i in range(prune_iters):
        ...
        if i in save_model_iter:
            save_model(best_model,[prune_type,dataset,prune_type,sensitive_group,network,accumulated_pruned, exp_idx])
    ...
    if prune_iters > 0 and retrain:
        ...
        best_model = train(best_model, criterion, dataloaders,lr_schedule, epoches,col_used_training, output_cols_each_task)        
    # Save model
    save_model(best_model,[prune_type,dataset,prune_type,sensitive_group,network,prune_rate, exp_idx])

    # Save prediction output
    fair_df = save_output(best_model,[dataset, prune_type,prune_rate, frames['test'], face_dir, total_classes, network, col_used, output_cols_each_task, sensitive_group, exp_idx],csv_savedir)

    # Print acc scores, overall and by groups
    if print_acc:
        show_acc_df(fair_df, fair_df_full, prune_type, col_used, sensitive_group)
