...
def make_model(pruning=True, network="resnet34", n_classes=18):
    if network == "resnet34":
        ...
        model_conv = models.resnet50(
            pretrained=True) if pruning else torchvision.models.resnet50(pretrained=True)
        num_ftrs = model_conv.fc.in_features
        model_conv.fc = nn.Linear(num_ftrs, n_classes)
...
def check_imgs(frame, new_img_dir):
    ...
    print("{} out of {} faces are found in new dir!".format(
        faces_found, initial_rows))
...
def check_fairness0(trained_model, test_frame, new_img_dir, n_classes=7, arch="resnet34", col_used=['race'], output_cols_each_task=[(0, 7)], sensitive_group="gender"):
    ...
    for index, row in tqdm(test_frame.iterrows()):
        ...
        for col, st_end in zip(col_used, output_cols_each_task):
            ...
            score_this_col = np.exp(outputs_this_col) / \
                np.sum(np.exp(outputs_this_col))
    ...
    fair_df_cols, fair_df_col_names = [face_names, list(test_frame[sensitive_group])], [
        'face_name_align', sensitive_group]
...
def print_acc_scores(fair_df, col_used=['race'], sensitive_group="gender"):
    ...
    for i, col in enumerate(col_used):
        acc_each_task[i] = sum(
            fair_df[col] == fair_df[col+"_preds_fair"]) / n_row
        for g in unq_groups:
            idx_g = fair_df[sensitive_group] == g
            n_g = sum(idx_g)
            acc_each_group[g][i] = sum(
                fair_df[col][idx_g] == fair_df[col+"_preds_fair"][idx_g]) / n_g

    acc_overall = sum(acc_each_task) / n_row
    acc_each_group = [sum(acc_each_group_task) / len(acc_each_group_task)
                      for acc_each_group_task in acc_each_group]

    return {"acc": acc_overall, "acc_each_group": acc_each_group}


def show_acc_df(fair_df, fair_df_full, prune_type, col_used, sensitive_group):
    ...
    acc_scores_full = [acc_scores_full['acc']] + \
        acc_scores_full['acc_each_group']
    acc_df = {"Type": ['No pruning', prune_type, "Difference"]}
    for i in range(len(acc_scores)):
        acc_df[i] = [acc_scores[i], acc_scores_full[i],
                     acc_scores[i] - acc_scores_full[i]]
    acc_df = pd.DataFrame(acc_df)
    acc_df.columns = ['Type', "Overall"] + \
        ['Group ' + str(i) for i in range(len(acc_scores))]
    print("Accuracy:")
    print(acc_df)
...
def save_model(best_model, cfgs):
    ...
    model_path = 'trained_model/{}/{}_{}_{}_by{}_{}_{}_{}.pt'.format(
        prune_type, dataset, prune_type, loss_type, sensitive_group, network, prune_rate, exp_idx)
...
def save_output(best_model, cfgs, csv_savedir="fair_dfs", save_file=True):
    ...
    fair_df = check_fairness0(best_model, test_frame, face_dir, total_classes,
                              network, col_used, output_cols_each_task, sensitive_group)
    df_path = '{}/{}_{}_{}_by{}_{}_{}_{}.csv'.format(
        csv_savedir, dataset, prune_type, loss_type, sensitive_group, network, prune_rate, exp_idx)
...
def download_dataset(dataset, img_dir):
    ...
    if os.path.isdir(img_dir):
        ...
        raise FileNotFoundError(
            "Imagenet not found. Request the person subtree through the database website, and save all images under Images/Imagenet")
